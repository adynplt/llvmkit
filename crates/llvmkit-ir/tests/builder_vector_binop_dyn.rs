...
fn vector_binops_emit_elementwise_ir() {
    ...
    let fn_ty = m.fn_type(void_ty.as_type(), [vec_ty.as_type(), vec_ty.as_type()], false);
    let f = m
    ...
    let shamt = vec_ty
        ...
        .expect("shamt vec");
    let _sh = b.build_int_shl_dyn(s, shamt.as_value(), "sh").expect("shl vec");

    b.build_ret_void();

    let txt = format!("{m}");
    assert!(txt.contains("xor <2 x i64>"), "expected vector xor, got:\n{txt}");
    assert!(txt.contains("add <2 x i64>"), "expected vector add, got:\n{txt}");
    assert!(txt.contains("shl <2 x i64>"), "expected vector shl, got:\n{txt}");
}
...
fn scalar_binop_dyn_still_works() {
    ...
    let i64_ty = m.i64_type();
    let fn_ty = m.fn_type(i64_ty.as_type(), [i64_ty.as_type(), i64_ty.as_type()], false);
    let f = m
